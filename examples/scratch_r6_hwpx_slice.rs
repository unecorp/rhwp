//! scratch: HWPX 문단 창 절단 축소본 생성기 (RHWP_SLICE_SRC/KEEP/OUT).
//! KEEP="secIdx:paraStart..paraEnd[,secIdx:...]" — 지정한 창 밖 문단을 버리고
//! 지정 밖 구역은 통째로 버린다. BinData 는 유지(필요 시 후처리).
...
fn main() {
    ...
    let mut doc = rhwp::parser::hwpx::parse_hwpx(&raw).unwrap();
    ...
    if std::env::var("RHWP_SLICE_STUB").is_ok() {
        // 1×1 투명 PNG — 위치/크기는 컨트롤 속성이 결정하므로 화소는 무관하다.
        const PNG1X1: &[u8] = &[
