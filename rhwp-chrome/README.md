...
- **HWPX 파일**: 열람·편집·저장 지원. HWPX→HWP 변환 저장도 지원 ([#197](https://github.com/edwardkim/rhwp/issues/197) 완료, v0.8.0 엔진)
...
- 미리보기 압축 해제에 출력 크기 상한과 ZIP metadata·실제 길이 일치 검증을 적용해 과대한
  메모리 사용을 차단
- 조판·폰트·저장 정확도와 편집 기능을 보강한 rhwp core v0.8.6 WASM 번들 반영
- v0.8.4 대비 새 확장 권한 및 새 외부 네트워크 endpoint 없음
...
- 저장 신뢰성 대공사 — 양식 값·책갈피·누름틀·스타일·표 테두리·수식 속성 등 편집 내용이 저장 시 유실·원복되던 문제 대량 수정
- HWPX 저장 왕복 보존 수십 건 (세로쓰기, 표 겹침, 테두리 입체/그림자, 화살표, 체크박스 상태 등)
- 편집 취소(undo) 정합 — 각주/미주/수식 삽입, 머리말/꼬리말 조작, 표 셀 문단 병합 복원
- 글꼴 처리 일원화·표시 정확도 개선 (1만 건 실문서 검증 쪽수 회귀 0), 손상/악성 파일 파서 방어 강화, 한글 입력 반응성 개선
...
- v0.2.7 후속 핫픽스 — 브라우저/서비스 워커 재시작 후 과거 다운로드 기록이 뷰어 탭을 다시 열던 문제를 시각 검증으로 추가 차단

### v0.2.7 (2026-06-24)

- v0.2.6 핫픽스 — 백그라운드 재시작 시 과거 다운로드 항목 재열림 차단 (onCreated 관찰분만 열기)

### v0.2.6 (2026-06-23)

- rhwp core `0.7.17` WASM 번들 반영
- 뷰어 CSP 준수 보강, 다른 확장과의 다운로드 동작 충돌 정정

### v0.2.5 (2026-06-19)

- rhwp core `0.7.16` WASM 번들 반영
- 로컬 파일 드래그&드롭에 보안 확인 단계 추가
