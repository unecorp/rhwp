//! 코퍼스 샤드 로더. 행 유일성(rowId · tuple · task+criterionId)을 강제한다.
...
pub fn load_shard_rows(path: &Path) -> Result<Vec<DecompRow>, LoadError> {
    let text =
    ...
    for (i, line) in text.lines().enumerate() {
        ...
        let row: DecompRow = serde_json::from_str(line)
...
pub fn load_shards(dir: &Path) -> Result<Vec<DecompRow>, LoadError> {
    ...
    for shard in &manifest.shards {
        ...
        for row in shard_rows {
            ...
            if !crits.insert(row.criterion_id.clone()) {
                return Err(LoadError::Duplicate(format!(
                    "criterionId {}",
                    row.criterion_id
                )));
            }
            if !row.task.trim().is_empty() && !tasks.insert(row.task.clone()) {
                return Err(LoadError::Duplicate(format!("task {}", row.row_id)));
            }
            let key = row.tuple_key();
            if !tuples.insert(key) {
                return Err(LoadError::Duplicate(format!("tuple {}", row.row_id)));
            }
            rows.push(row);
        }
    }
