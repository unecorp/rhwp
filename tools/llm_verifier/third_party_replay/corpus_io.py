"""TSV shard IO for the committed V-replay corpus."""
...
try:
    from .schema import CASE_COLUMNS, ReplayCase
...
def tsv_line(case: ReplayCase) -> str:
    row = case.to_row()
    return "\t".join(row[col] for col in CASE_COLUMNS)


def write_shard(path: Path, cases: Iterable[ReplayCase]) -> int:
...
def load_shard(path: Path) -> list[ReplayCase]:
    rows = parse_tsv(path.read_text(encoding="utf-8"))
    return [ReplayCase.from_mapping(row) for row in rows]
...
def load_corpus(dir_path: Path | None = None) -> list[ReplayCase]:
    root = dir_path or CORPUS_DIR
