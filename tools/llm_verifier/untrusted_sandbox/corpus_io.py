"""Read committed V-nonce sandbox TSV shards."""
...
from .schema import CASE_COLUMNS, SandboxCase
...
def iter_tsv(path: Path) -> Iterator[SandboxCase]:
    ...
    for lineno, line in enumerate(lines[1:], start=2):
        ...
        if len(cells) != len(CASE_COLUMNS):
            raise ValueError(
                f"{path}:{lineno} expected {len(CASE_COLUMNS)} cells, got {len(cells)}"
            )
        yield SandboxCase.from_mapping(dict(zip(CASE_COLUMNS, cells, strict=True)))


def iter_corpus(corpus_dir: Path | None = None) -> Iterator[SandboxCase]:
    ...
    for shard in manifest["shards"]:
        ...
        if not path.is_absolute():
            path = HERE / rel if rel.startswith("corpus/") else root / path.name
