# HWP5 Table Probe Plan

## Input

- oracle: `samples/hwpx/hancom-hwp/hwpx-h-01-all-table-axes.hwp`
- generated: `output/poc/hwpx2hwp/inventory/hwpx-h-01-all-table-axes.hwp`
- align_mode: `lcs`
- report_mode: `table-probe-plan`
- candidate_count: `2`

## Probe Axes

| axis | record kind | affected records | meaning |
|---|---|---:|---|
| `ctrl_outer_margin` | `CTRL_HEADER(Table)` | 1 | TABLE control wrapper의 바깥 여백 4필드 |
| `ctrl_common_attr` | `CTRL_HEADER(Table)` | 1 | TABLE control wrapper 공통 속성 비트 |
| `table_attr` | `TABLE` | 1 | TABLE record 첫 4바이트 속성 비트 |
| `table_tail` | `TABLE` | 1 | TABLE record 0x16 이후 tail payload |

## Recommended Probe Matrix

| variant | graft axes | purpose |
|---|---|---|
| `01_ctrl_outer_margin_only` | `ctrl_outer_margin` | TABLE CTRL_HEADER outer margin 단독 효과 확인 |
| `02_table_attr_only` | `table_attr` | TABLE attr 단독 효과 확인 |
| `03_table_tail_only` | `table_tail` | TABLE payload tail 단독 효과 확인 |
| `04_ctrl_common_attr_only` | `ctrl_common_attr` | TABLE CTRL_HEADER common attr 차이의 영향 확인 |
| `05_outer_margin_table_attr` | `ctrl_outer_margin, table_attr` | 위치/속성 축 결합 확인 |
| `06_outer_margin_table_tail` | `ctrl_outer_margin, table_tail` | 위치/tail 축 결합 확인 |
| `07_table_attr_tail` | `table_attr, table_tail` | TABLE record 내부 축 결합 확인 |
| `08_all_table_axes` | `ctrl_outer_margin, ctrl_common_attr, table_attr, table_tail` | TABLE 관련 관찰 축 전체 positive guard |

## Axis Details

### `ctrl_outer_margin`

- record_kind: `CTRL_HEADER(Table)`
- affected_records: `1`
- description: TABLE control wrapper의 바깥 여백 4필드

| key | oracle record | generated record | fields | oracle values | generated values |
|---|---|---|---|---|---|
| `BodyText.Section0#4` | `BodyText.Section0#4` | `BodyText.Section0#4` | `out_margin_left, out_margin_right, out_margin_top, out_margin_bottom` | `out_margin_left=80; out_margin_right=80; out_margin_top=40; out_margin_bottom=40` | `out_margin_left=0; out_margin_right=0; out_margin_top=0; out_margin_bottom=0` |

### `ctrl_common_attr`

- record_kind: `CTRL_HEADER(Table)`
- affected_records: `1`
- description: TABLE control wrapper 공통 속성 비트

| key | oracle record | generated record | fields | oracle values | generated values |
|---|---|---|---|---|---|
| `BodyText.Section0#4` | `BodyText.Section0#4` | `BodyText.Section0#4` | `common_attr` | `common_attr=0x00000001 (1)` | `common_attr=0x00000000 (0)` |

### `table_attr`

- record_kind: `TABLE`
- affected_records: `1`
- description: TABLE record 첫 4바이트 속성 비트

| key | oracle record | generated record | fields | oracle values | generated values |
|---|---|---|---|---|---|
| `BodyText.Section0#5` | `BodyText.Section0#5` | `BodyText.Section0#5` | `table_attr` | `table_attr=0x00000003 (3)` | `table_attr=0x00000000 (0)` |

### `table_tail`

- record_kind: `TABLE`
- affected_records: `1`
- description: TABLE record 0x16 이후 tail payload

| key | oracle record | generated record | fields | oracle values | generated values |
|---|---|---|---|---|---|
| `BodyText.Section0#5` | `BodyText.Section0#5` | `BodyText.Section0#5` | `table_tail_full` | `table_tail_full=2 bytes: ab cd` | `table_tail_full=2 bytes: 00 00` |

## Notes

- 이 문서는 판정용 HWP를 직접 생성하지 않는다. 다음 단계에서 바이너리 graft 또는 저장기 projection을 만들 때 사용할 작업 지시서다.
- 필드명은 현재 P0 decoder의 관찰명이다. `tail_after_0x16`, `z_order_or_instance` 등은 contract 이름으로 확정하지 않는다.
- probe 생성 시에는 한 번에 한 축만 바꾸는 파일과 전체 positive guard 파일을 함께 만들어야 한다.
- 페이지 수 로직은 이 계획이 바꾸지 않는다. `rhwp_pages` 칸은 관측 메모일 뿐 serializer 입력이 아니다.
