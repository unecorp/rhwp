...
명령: `rhwp export-pdf 공문.hwp -o output/poc/cli/공문.pdf --profile print`

이 예제는 기존 `export-pdf` 만 쓴다. 새 CLI 없음. gym 없음.

## 언제

사용자가 "인쇄 PDF" 에 해당하는 말을 할 때. 페이지가 있으면 0 기준인지 확인한다.

## 절차

```bash
cargo build --release
rhwp export-pdf 공문.hwp -o output/poc/cli/공문.pdf --profile print
```
