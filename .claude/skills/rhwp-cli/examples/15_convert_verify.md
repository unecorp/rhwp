...
명령: `rhwp convert 배포.hwp 편집.hwp --verify --verify-pages`

이 예제는 기존 `convert` 만 쓴다. 새 CLI 없음. gym 없음.

## 언제

사용자가 "배포용 해제" 에 해당하는 말을 할 때. 페이지가 있으면 0 기준인지 확인한다.

## 절차

```bash
cargo build --release
rhwp convert 배포.hwp 편집.hwp --verify --verify-pages
```
