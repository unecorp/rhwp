...
명령: `rhwp export-png 공문.hwp -p 0 --vlm-target claude -o output/poc/cli/png`

이 예제는 기존 `export-png` 만 쓴다. 새 CLI 없음. gym 없음.

## 언제

사용자가 "VLM PNG" 에 해당하는 말을 할 때. 페이지가 있으면 0 기준인지 확인한다.

## 절차

```bash
cargo build --release
rhwp export-png 공문.hwp -p 0 --vlm-target claude -o output/poc/cli/png
```
