...
명령: `rhwp export-markdown 공문.hwp -p 0 -o output/poc/cli/md`

이 예제는 기존 `export-markdown` 만 쓴다. 새 CLI 없음. gym 없음.

## 언제

사용자가 "MD" 에 해당하는 말을 할 때. 페이지가 있으면 0 기준인지 확인한다.

## 절차

```bash
cargo build --release
rhwp export-markdown 공문.hwp -p 0 -o output/poc/cli/md
```
