...
명령: `rhwp export-svg 공문.hwp --debug-overlay -p 2 -o output/poc/cli/overlay`

이 예제는 기존 `export-svg` 만 쓴다. 새 CLI 없음. gym 없음.

## 언제

사용자가 "겹침 overlay" 에 해당하는 말을 할 때. 페이지가 있으면 0 기준인지 확인한다.

## 절차

```bash
cargo build --release
rhwp export-svg 공문.hwp --debug-overlay -p 2 -o output/poc/cli/overlay
```
