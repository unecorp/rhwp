...
권위: `mydocs/manual/cli_commands.md` 의 `export-svg` 절, `src/main.rs` 디스패치.
새 플래그를 발명하지 않는다. 여기 없는 옵션은 `--help` 와 매뉴얼을 본다.

## 한 줄

`rhwp export-svg <파일> [-p N] [-o 폴더] [--debug-overlay] [--json] [--profile print|screen]`

## 요청 매핑

- "SVG로 내보내" → `export-svg`
- "시각 확인" → `export-svg`
- "debug overlay" → `export-svg`
- "겹침 보이게" → `export-svg`
...
```bash
rhwp export-svg samples/basic/KTX.hwp -p 0
```
...
관련: [00_tree.md](00_tree.md) · [01_request_command_map.md](01_request_command_map.md) · 장 02

## 호출 카드

| 상황 | 명령 | 읽는 것 |
|---|---|---|
| 없는 경로 | `rhwp export-svg 없는파일.hwp` | exit 1, missing-file |
| 인자 없음 | `rhwp export-svg` | exit 2, no-input (인자가 필수인 명령) |
| 한컴 4쪽 | `rhwp export-svg doc.hwp -p 3` | 0 기준 |
| JSON 실패 | `rhwp export-svg broken.hwp --json` | stdout 0바이트 |
