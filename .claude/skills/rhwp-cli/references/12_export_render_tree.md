...
권위: `mydocs/manual/cli_commands.md` 의 `export-render-tree` 절, `src/main.rs` 디스패치.
새 플래그를 발명하지 않는다. 여기 없는 옵션은 `--help` 와 매뉴얼을 본다.

## 한 줄

`rhwp export-render-tree <파일> [-p N] [-o 폴더]`

## 요청 매핑

- "bbox" → `export-render-tree`
- "render tree" → `export-render-tree`
- "좌표 JSON" → `export-render-tree`
...
```bash
rhwp export-render-tree samples/basic/KTX.hwp -p 0
```
...
관련: [00_tree.md](00_tree.md) · [01_request_command_map.md](01_request_command_map.md) · 장 12

## 호출 카드

| 상황 | 명령 | 읽는 것 |
|---|---|---|
| 없는 경로 | `rhwp export-render-tree 없는파일.hwp` | exit 1, missing-file |
| 인자 없음 | `rhwp export-render-tree` | exit 2, no-input (인자가 필수인 명령) |
| 한컴 4쪽 | `rhwp export-render-tree doc.hwp -p 3` | 0 기준 |
