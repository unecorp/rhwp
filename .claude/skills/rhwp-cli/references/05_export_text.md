...
권위: `mydocs/manual/cli_commands.md` 의 `export-text` 절, `src/main.rs` 디스패치.
새 플래그를 발명하지 않는다. 여기 없는 옵션은 `--help` 와 매뉴얼을 본다.

## 한 줄

`rhwp export-text <파일> [-p N] [-o 폴더] [--json] [--max-chars N]`
...
```bash
rhwp export-text samples/basic/KTX.hwp -p 0
```
...
관련: [00_tree.md](00_tree.md) · [01_request_command_map.md](01_request_command_map.md) · 장 05

## 호출 카드

| 상황 | 명령 | 읽는 것 |
|---|---|---|
| 없는 경로 | `rhwp export-text 없는파일.hwp` | exit 1, missing-file |
| 인자 없음 | `rhwp export-text` | exit 2, no-input (인자가 필수인 명령) |
| 한컴 4쪽 | `rhwp export-text doc.hwp -p 3` | 0 기준 |
| JSON 실패 | `rhwp export-text broken.hwp --json` | stdout 0바이트 |
