...
권위: `mydocs/manual/cli_commands.md` 의 `convert` 절, `src/main.rs` 디스패치.
새 플래그를 발명하지 않는다. 여기 없는 옵션은 `--help` 와 매뉴얼을 본다.

## 한 줄

`rhwp convert <입력> <출력.hwp> [--verify] [--verify-pages]`
...
```bash
rhwp convert samples/basic/KTX.hwp
```
...
관련: [00_tree.md](00_tree.md) · [01_request_command_map.md](01_request_command_map.md) · 장 15

## 호출 카드

| 상황 | 명령 | 읽는 것 |
|---|---|---|
| 없는 경로 | `rhwp convert 없는파일.hwp` | exit 1, missing-file |
| 인자 없음 | `rhwp convert` | exit 2, no-input (인자가 필수인 명령) |
| 한컴 4쪽 | `rhwp convert doc.hwp -s 0 -p 3` | 0 기준 |
