...
권위: `mydocs/manual/cli_commands.md` 의 `ir-diff` 절, `src/main.rs` 디스패치.
새 플래그를 발명하지 않는다. 여기 없는 옵션은 `--help` 와 매뉴얼을 본다.

## 한 줄

`rhwp ir-diff <a.hwpx> <b.hwp> [-s N] [-p M] [--summary] [--json]`

## 요청 매핑

- "IR 비교" → `ir-diff`
- "HWPX와 HWP 차이" → `ir-diff`
- "변환이 같은지" → `ir-diff`
...
```bash
rhwp ir-diff samples/basic/KTX.hwp
```
...
관련: [00_tree.md](00_tree.md) · [01_request_command_map.md](01_request_command_map.md) · 장 13

## 호출 카드

| 상황 | 명령 | 읽는 것 |
|---|---|---|
| 없는 경로 | `rhwp ir-diff 없는파일.hwp` | exit 1, missing-file |
| 인자 없음 | `rhwp ir-diff` | exit 2, no-input (인자가 필수인 명령) |
| 한컴 4쪽 | `rhwp ir-diff doc.hwp -s 0 -p 3` | 0 기준 |
| JSON 실패 | `rhwp ir-diff broken.hwp --json` | stdout 0바이트 |
